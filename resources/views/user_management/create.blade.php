...
<div class="main-content" style="padding:2rem;background:#f3f4f6;min-height:100vh;">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:2rem;">
        <h1 style="color:#111827;font-size:1.875rem;font-weight:600;margin:0;">Create New User</h1>
    ...
    <div class="card" style="background:white;border-radius:0.75rem;box-shadow:0 4px 6px -1px rgba(0,0,0,0.1),0 2px 4px -1px rgba(0,0,0,0.06);overflow:hidden;max-width:800px;margin:0 auto;">
        <div class="card-header" style="padding:1.25rem 1.5rem;background:white;border-bottom:1px solid #e5e7eb;">
            <h3 style="margin:0;color:#111827;font-size:1.125rem;font-weight:600;">Create New User Account</h3>
        </div>
        <div class="card-body" style="padding:1.5rem;">
            <form method="POST" action="{{ route('user-management.store') }}" style="display:grid;gap:1.5rem;">
                @csrf
                ...
                <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:1rem;">
                    ...
                    <div>
                        ...
                        <select name="role" 
                            ...
                            <option value="user" {{ old('role') === 'user' ? 'selected' : '' }}>User (Warehouse Staff)</option>
                            <option value="admin" {{ old('role') === 'admin' ? 'selected' : '' }}>Admin (Manager)</option>
                            @if(auth()->check() && auth()->user()->role === 'super_admin')
                                <option value="super_admin" {{ old('role') === 'super_admin' ? 'selected' : '' }}>Super Admin (Owner)</option>
                            @endif
                        </select>
                        @error('role')
                            <div style="color:#dc2626;font-size:0.75rem;margin-top:0.25rem;">{{ $message }}</div>
                ...
                <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:1rem;">
                    <div>
                        ...
                        <div style="color:#6b7280;font-size:0.75rem;margin-top:0.25rem;">Minimum 8 characters</div>
                        @error('password')
                            <div style="color:#dc2626;font-size:0.75rem;margin-top:0.25rem;">{{ $message }}</div>
                        @enderror
                    </div>
